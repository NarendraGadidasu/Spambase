...
res1 = res1.loc[res1['param_KNN__metric'] == 'manhattan', :]
...
lns1 = ax1.plot(res1['param_KNN__n_neighbors'], res1['CV_balanced_accuracy'], label = 'CV_balanced_accuracy', c = 'r')
...
plt.title('Manhattan weighted distance (Spambase)')

plt.legend()

plt.savefig('{}_Acc_wrt_n_neighbors_manhattan.png'.format(algo))
...
plt.title('Learning curve wrt Number of training samples (Spambase)')
...
plt.title('fit_or_score_time_wrt_fraction_of_data (Spambase)')
