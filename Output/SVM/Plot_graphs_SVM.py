...
res1 = res.loc[res['param_SVM__gamma_frac'] == 1.8, :]

res1 = res1.loc[res1['param_SVM__n_iter'] == 389, :]
...
plt.title('gamma_frac : 1.8 and n_iter : 389 (Spambase_SVM_RBF)')

plt.savefig('{}_Acc_wrt_Alpha.png'.format(algo))

plt.clf()

res2 = res.loc[res['param_SVM__alpha'] == 0.01, :]

res2 = res2.loc[res['param_SVM__n_iter'] == 389, :]
...
plt.title('alpha : 0.01 and n_iter : 389 (Spambase_SVM_RBF)')
...
plt.title('Learning curve wrt Number of training samples (Spambase_SVM_RBF)')
...
lc_iter = pd.read_csv('ITER_base_{}_{}_Overview.csv'.format(algo,data))
...
plt.title('Learning curve wrt Number of iterations (Spambase_SVM_RBF)')
...
plt.title('fit_or_score_time_wrt_fraction_of_data (Spambase_SVM_RBF)')
...
res1 = res.loc[res['param_SVM__n_iter'] == 389, :]
...
plt.title('n_iter : 389 (Spambase_SVM_Lin)')
...
plt.title('Learning curve wrt Number of training samples (Spambase_SVM_Lin)')
...
plt.title('Learning curve wrt Number of iterations (Spambase_SVM_Lin)')
...
plt.title('fit_or_score_time_wrt_fraction_of_data (Spambase_SVM_Lin)')
